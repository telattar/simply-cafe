...
export const menuController = {
  ...
  async addBundle({ bundleId, availability, stockCount }) {
    try {
      ...
      const bundleExists = await Menu.findOne({
        "bundle._id": bundleId,
      }).lean();
      if (bundleExists)
  ...
  async updateItemStock({ itemId, stockCount }) {
    try {
      ...
      if (stockCount < 0)
        throw new APIError(
          BAD_REQUEST,
          "Please provide a stock count that is greater than zero."
        );

      const availability = stockCount > 0 ? true : false;
      const updatedMenuItem = await Menu.updateOne(
        { "item._id": itemId },
  ...
  async deleteItem({ itemId }) {
    try {
      const deletedItem = await Menu.deleteOne({ "item._id": itemId }).lean();
  ...
  async deleteBundle({ bundleId }) {
    try {
      const deletedBundle = await Menu.deleteOne({
        "bundle._id": bundleId,
      }).lean();
